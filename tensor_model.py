...
def define_Caption_Model(batchsize,max_voc_plus_1,max_sen_len,tokenizer):
    ...
    with tf.variable_scope('INPUT'):
        input1 = tf.placeholder(tf.int32,shape=[None,max_sen_len])
        input2 = tf.placeholder(tf.float32, shape=[None, 4096])
    # compare 2, different embedding layer
    with tf.variable_scope('RNN'):
        #embedding_parm = tf.get_variable('embedding',shape=[max_voc_plus_1,100],initializer=tf.random_uniform_initializer())
        embedding_parm = tf.constant(embedding_layer(max_voc_plus_1,tokenizer,max_sen_len),dtype=tf.float32)
        word_embedding = tf.nn.embedding_lookup(embedding_parm,input1)
        time_steps = max_sen_len
        lstm = tf.contrib.rnn.BasicLSTMCell(100)
        initial_state = lstm.zero_state(batchsize, dtype=tf.float32)
    ...
    with tf.variable_scope('Dense1'):
        ...
        bias = tf.get_variable(name='bias',initializer=tf.zeros([100]))
    ...
    with tf.variable_scope('Dense2'):
        ...
        bias = tf.get_variable(name='bias',initializer=tf.zeros([1024]))
    ...
    with tf.variable_scope('Dense3'):
        ...
        bias = tf.get_variable(name='bias',initializer=tf.zeros([max_voc_plus_1]))
...
def train(batchsize,trainx,trainIMG,trainy,testx,testIMG,testy,learningrate,epoch,max_voc_plus_1,max_sen_len,tokenizer):
    ...
    y = tf.placeholder(tf.float32,shape=[None,7633])
...
def get_IMG_features(img):
	cnnmodel = load_model(os.path.join('CNNMODELS','VGG16.h5'))
	img = imread(img)
...
def getsomeinput():
    ...
    return trainX,trainXIMG,trainY,testX,testXIMG,testY,max_voc_plus_1,max_sen_len,tokenizer
