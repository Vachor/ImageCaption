...
def define_Caption_Model(batchsize,max_voc_plus_1,max_sen_len,tokenizer):
    ...
    with tf.variable_scope('INPUT'):
        ...
        input2 = tf.placeholder(np.float32, shape=[None, 4096])
    dropout2 = Dropout(0.4)(input2)
    with tf.variable_scope('Dense1'):
        weight = tf.get_variable(name='weight',shape=[4096,256],initializer=tf.glorot_uniform_initializer())
        bias = tf.get_variable(name='bias',initializer=tf.random_normal([256]))
        dense1 = tf.matmul(dropout2,weight) + bias
        dense1 = tf.nn.relu(dense1)
        dense1 = tf.reshape(dense1,shape=[-1,1,256])
    # compare 2, different embedding layer
    with tf.variable_scope('RNN'):
        #embedding = Embedding(max_voc_plus_1,256,mask_zero=True)(input1)
        embedding_parm = tf.get_variable('embedding',shape=[max_voc_plus_1,256],initializer=tf.glorot_uniform_initializer())
    ...
    with tf.variable_scope('Dense2'):
        weight = tf.get_variable(name='weight', shape=[256, 1024], initializer=tf.glorot_uniform_initializer())
...
def get_IMG_features(img):
	cnnmodel = load_model(os.path.join('modelss','VGG16.h5'))
	img = imread(img)
