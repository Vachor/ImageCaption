...
def embedding_layer(max_voc_plus_1,tokenizer,max_sen_len):
    ...
    embedding_matrix = np.zeros((max_voc_plus_1,100))
    ...
    #embedding_l = Embedding(max_voc_plus_1,100,embeddings_initializer=Constant(embedding_matrix),input_length=max_sen_len,trainable=False)
...
def define_Caption_Model(batchsize,max_voc_plus_1,max_sen_len,tokenizer):
    ...
    with tf.variable_scope('Dense1'):
        weight = tf.get_variable(name='weight',shape=[2048,100],initializer=tf.glorot_uniform_initializer())
        bias = tf.get_variable(name='bias',initializer=tf.random_normal([100]))
        dense1 = tf.matmul(dropout2,weight) + bias
        dense1 = tf.nn.relu(dense1)
        dense1 = tf.reshape(dense1,shape=[-1,1,100])
    # compare 2, different embedding layer
    with tf.variable_scope('RNN'):
        #embedding = Embedding(max_voc_plus_1,100,mask_zero=True)(input1)
        #embedding_parm = tf.constant(embedding_layer(max_voc_plus_1, tokenizer, max_sen_len), dtype=np.float32)
        embedding_parm = tf.get_variable('embedding',shape=[max_voc_plus_1,100],initializer=tf.glorot_uniform_initializer())
    ...
    #     embedding = tf.Variable(np.identity(100, dtype=np.float32))
    ...
    #Dense1 = Dense(100,activation='relu')(dropout2)
    with tf.variable_scope('Dense2'):
        weight = tf.get_variable(name='weight', shape=[100, 1024], initializer=tf.glorot_uniform_initializer())
