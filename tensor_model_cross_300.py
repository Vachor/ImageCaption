...
def embedding_layer(max_voc_plus_1,tokenizer,max_sen_len):
    ...
    with open(os.path.join('glove.6B','glove.6B.300d.txt'),'r',encoding='utf-8') as f:
        for line in f:
            values = line.split()
            word = values[0]
            coefs = np.asarray(values[1:],dtype='float32')
            embedding_index[word] = coefs
    embedding_matrix = np.zeros((max_voc_plus_1,300))
   ...
   # embedding_l = Embedding(max_voc_plus_1,300,embeddings_initializer=Constant(embedding_matrix),input_length=max_sen_len,trainable=False)
    return embedding_matrix
def InsertTensor(dense,word_embedding,loc):
    a = word_embedding[:,0:loc,:]
    temp = word_embedding[:,loc:,:]
    b = tf.concat([dense,temp],axis=1)
...
def define_Caption_Model(batchsize,max_voc_plus_1,max_sen_len,tokenizer):
    ...
    with tf.variable_scope('Dense1'):
        weight = tf.get_variable(name='weight',shape=[2048,300],initializer=tf.glorot_uniform_initializer())
        bias = tf.get_variable(name='bias',initializer=tf.random_normal([300]))
        dense1 = tf.matmul(dropout2,weight) + bias
        dense1 = tf.nn.relu(dense1)
        dense1 = tf.reshape(dense1,shape=[-1,1,300])
    # compare 2, different embedding layer
    with tf.variable_scope('RNN'):
        #embedding = Embedding(max_voc_plus_1,300,mask_zero=True)(input1)
        #embedding_parm = tf.constant(embedding_layer(max_voc_plus_1, tokenizer, max_sen_len), dtype=np.float32)
        embedding_parm = tf.get_variable('embedding',shape=[max_voc_plus_1,300],initializer=tf.glorot_uniform_initializer())
        word_embedding = tf.nn.embedding_lookup(embedding_parm,input1)
        word_embedding = tf.concat([dense1,word_embedding],axis=1)

        for i in range(1,38):
            word_embedding = InsertTensor(dense1,word_embedding,i*2)

        word_embedding = tf.nn.dropout(word_embedding,0.4)
        time_steps = 76
        lstm = tf.contrib.rnn.BasicLSTMCell(300)
    ...
    #     embedding = tf.Variable(np.identity(300, dtype=np.float32))
    ...
    #dropout1 = Dropout(0.4)(embedding)
        ...
        final_out = out
        #lstm = LSTM(300)(dropout1)
    #input2 = Input(shape=(4096,))

    #Dense1 = Dense(300,activation='relu')(dropout2)
    with tf.variable_scope('Dense2'):
        weight = tf.get_variable(name='weight', shape=[300, 1024], initializer=tf.glorot_uniform_initializer())
        bias = tf.get_variable(name='bias',initializer=tf.random_normal([1024]))
        dense2 = tf.matmul(final_out, weight) + bias
    ...
    return dense3,input1,input2,dense2,final_out,word_embedding,lstm,dense1,initial_state,current_state
...
def train(batchsize,trainx,trainIMG,trainy,testx,testIMG,testy,learningrate,epoch,max_voc_plus_1,max_sen_len,tokenizer):
    ...
    for i in range(epoch):
        ...
        for start,end in zip(range(0,len(trainx),batchsize),range(batchsize,len(trainx),batchsize)):
            ...
            _,loss_value,next_state = sess.run([optimizer,loss,final_state],feed_dict={input1:trainx[index[start:end]],input2:trainIMG[index[start:end]],y:trainy[index[start:end]],initial_state:next_state})
            print("Current Cost: ", loss_value, "\t Epoch {}/{}".format(i, epoch),
                    "\t Iter {}/{}".format(start, len(trainx)))

        # for start,end in zip(range(0,len(testx),bachsize),range(bachsize,len(testx),bachsize)):
        #     val_loss = sess.run(loss,feed_dict={input1:testx[start:end],input2:testIMG[start:end],y:testy[start:end]})
        #     val_total += val_loss
        #     count += 1
        ...
        #val_total = 0
            # if counter % 100 == 0:
            #     img_features = get_IMG_features('example.jpg')
            #     opt = ''
            #     step = 0
            #     caption = ['startsen']
            #     str = ' '.join(caption)
...
def main():
    ...
    # train(128,trainX,trainXIMG,trainY,testX,testXIMG,testY,0.001,10,max_voc_plus_1,max_sen_len,tokenizer)
    totalc = 6
    bleu1 = 0
    bleu2 = 0
    bleu3 = 0
