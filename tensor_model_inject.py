...
def embedding_layer(max_voc_plus_1,tokenizer,max_sen_len):
    ...
    with open(os.path.join('glove.6B','glove.6B.50.txt'),'r',encoding='utf-8') as f:
        for line in f:
            values = line.split()
            word = values[0]
...
def define_Caption_Model(batchsize,max_voc_plus_1,max_sen_len,tokenizer):
    ...
    with tf.variable_scope('Dense1'):
        weight = tf.get_variable(name='weight',shape=[4096,50],initializer=tf.glorot_uniform_initializer())
        bias = tf.get_variable(name='bias',initializer=tf.random_normal([50]))
        dense1 = tf.matmul(dropout2,weight) + bias
        dense1 = tf.nn.relu(dense1)
        dense1 = tf.reshape(dense1,shape=[-1,1,50])
    # compare 2, different embedding layer
    with tf.variable_scope('RNN'):
        #embedding = Embedding(max_voc_plus_1,50,mask_zero=True)(input1)
    ...
    #     embedding = tf.Variable(np.identity(50, dtype=np.float32))
